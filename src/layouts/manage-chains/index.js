...
function ManageChains() {
  ...
  const fetchGroups = async () => {
    try {
      const response = await fetch("https://ims-backend-production-5193.up.railway.app/api/groups");
  ...
  const fetchChains = async () => {
    try {
      const response = await fetch("https://ims-backend-production-5193.up.railway.app/api/chains");
      const data = await response.json();
      setChains(data);
    } catch (err) {
  ...
  const handleDeleteChain = async (id) => {
    if (window.confirm("Are you sure you want to delete this chain?")) {
      await fetch(`https://ims-backend-production-5193.up.railway.app/api/chains/${id}`, { method: "DELETE" });
      fetchChains();
    }
  };
