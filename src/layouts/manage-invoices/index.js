...
function ManageInvoices() {
  ...
  const fetchInvoices = async () => {
    try {
      const response = await fetch("https://ims-backend-production-5193.up.railway.app/api/invoices");
  ...
  const submitPayment = async () => {
    ...
    try {
      await fetch(`https://ims-backend-production-5193.up.railway.app/api/invoices/${selectedInvoice.id}/confirm-payment`, {
        method: 'PUT',
  ...
  const deletePermanently = async (id) => {
    ...
    try {
      await fetch(`https://ims-backend-production-5193.up.railway.app/api/invoices/${id}`, { method: 'DELETE' });
      fetchInvoices();
  ...
  const downloadPdf = async (id) => {
    try {
      const res = await fetch(`https://ims-backend-production-5193.up.railway.app/api/invoices/${id}/pdf`);
  ...
  const processedInvoices = useMemo(() => {
    return invoices.filter((inv) => {
      ...
      const zoneName = String(inv.zoneName || "").toLowerCase(); // Added Zone search

      return groupName.includes(term) || invoiceNo.includes(term) || clientName.includes(term) || zoneName.includes(term);
    });
    ...
    <DashboardLayout>
      ...
      <MDBox pt={6} pb={3}>
        <MDBox display="flex" justifyContent="space-between" mb={3} bgColor="white" p={2} borderRadius="lg" shadow="sm">
          ...
          <MDBox width="250px">
            <MDInput
              placeholder="Search Client, ID, or Zone..."
              value={search}
        ...
        <Card>
          <MDBox p={3}>
            {processedInvoices.length === 0 ? (
              <MDTypography textAlign="center" py={3} color="text">No invoices found.</MDTypography>
            ) : (
                ...
                return (
                  <MDBox key={inv.id} mb={2} p={2} sx={{ border: "1px solid #eee", borderRadius: "8px", background: showBackups ? "#fff0f0" : "white" }}>
                    <Grid container alignItems="center">
                      {/* ✅ ENHANCED COLUMN: Now shows Client, Group, and Zone */}
                      <Grid item xs={12} md={5}>
                        <MDBox display="flex" alignItems="baseline">
                          <MDTypography variant="button" fontWeight="bold" fontSize="1.1rem" mr={1}>#{inv.invoiceNo}</MDTypography>
                          <MDTypography variant="button" fontWeight="medium" color="dark">{inv.clientName || inv.chain?.chainName || "Unknown Client"}</MDTypography>
                        </MDBox>
                        <MDTypography variant="caption" display="block" color="text" fontWeight="regular">
                          <Icon sx={{ fontSize: "small", verticalAlign: "middle", mr: 0.5 }}>business</Icon>
                          {inv.groupName} |
                          <Icon sx={{ fontSize: "small", verticalAlign: "middle", mx: 0.5 }}>location_on</Icon>
                          {inv.zoneName || "N/A"}
                        </MDTypography>
                      </Grid>

                      <Grid item xs={6} md={1} textAlign="center"><StatusLabel text={status} color={color} /></Grid>

                      <Grid item xs={6} md={3} textAlign="right" pr={2}>
                        <MDTypography variant="caption" display="block">Total: {formatMoney(total)}</MDTypography>
                        <MDTypography variant="button" color={balance > 0 ? "error" : "success"} fontWeight="bold">
                          {balance > 0 ? `Due: ${formatMoney(balance)}` : "Fully Paid"}
                        </MDTypography>
                      </Grid>

                      <Grid item xs={12} md={3} textAlign="right">
                        {!showBackups && balance > 0 && (
                          <MDButton variant="text" color="success" onClick={() => handleOpenPayment(inv)} title="Pay"><Icon>payment</Icon></MDButton>
                        )}
                        <MDButton variant="text" color="info" onClick={() => downloadPdf(inv.id)} title="Download"><Icon>download</Icon></MDButton>
                        <MDButton variant="text" color="warning" onClick={() => {
                          if(window.confirm(showBackups ? "Restore?" : "Archive?")) {
                            ...
                            fetch(`https://ims-backend-production-5193.up.railway.app/api/invoices/${inv.id}/${end}`, { method: 'PUT' }).then(fetchInvoices);
                          }
                        }} title="Move"><Icon>{showBackups ? "restore" : "archive"}</Icon></MDButton>
                        {showBackups && role === "admin" && (
                          <MDButton variant="text" color="error" onClick={() => deletePermanently(inv.id)} title="Delete"><Icon>delete_forever</Icon></MDButton>
                        )}
                      </Grid>
                    </Grid>
                  </MDBox>
