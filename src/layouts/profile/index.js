...
import Switch from "@mui/material/Switch"; // ✅ Restored for Settings
            ...
            {activeTab === 0 && (
                <Grid container spacing={3}>
                    <Grid item xs={12} md={8}>
                        <Card><MDBox p={3}>
                            <MDBox display="flex" justifyContent="space-between" mb={2}><MDTypography variant="h6">Team Attendance</MDTypography><MDButton size="small" onClick={fetchAdminData}>Refresh</MDButton></MDBox>
                            <List>
                    ...
                    <Grid item xs={12} md={4}>
                        <Card sx={{height: '100%', textAlign: 'center', p: 3}}><Icon fontSize="large">groups</Icon><MDTypography variant="h3">{employeeAttendance.length}</MDTypography><MDTypography variant="button">Present</MDTypography></Card>
                    </Grid>
                </Grid>
            )}
                            ...
                            <List>
                                {leaveRequests.map((req) => (
                                    <ListItem key={req.id} divider>
                                        ...
                                        <MDButton size="small" color="error" onClick={() => handleLeaveAction(req.id, 'reject')} sx={{ml:1}}>Reject</MDButton>
                                    </ListItem>
                                ))}
                            </List>
                        )}
...
const StaffDashboard = ({ activeTab, user }) => {
    ...
    const handleAttendance = () => {
        ...
        if (isCheckedIn) {
            setIsCheckedIn(false);
            localStorage.removeItem("checkInTime");
            Swal.fire("Checked Out", "See you tomorrow!", "success");
        } else {
            setIsCheckedIn(true);
            ...
            Swal.fire("Checked In", `Welcome back at ${nowTime}`, "success");
        }
    };
                ...
                <Grid container spacing={3} justifyContent="center">
                    <Grid item xs={12} md={6}>
                        <Card sx={{ height: "100%", textAlign: "center", p: 4 }}>
                            ...
                            <MDButton variant="gradient" color={isCheckedIn ? "warning" : "success"} size="large" circular sx={{ width: 150, height: 150, my: 3 }} onClick={handleAttendance}>
...
function Profile() {
    ...
    useEffect(() => {
        ...
        // 3. (Optional) Force Admin if email contains "admin" (Emergency Fix)
