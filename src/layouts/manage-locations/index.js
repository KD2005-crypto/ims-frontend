...
function ManageLocations() {
  ...
  const fetchBrands = async () => {
    try {
      const response = await fetch("https://ims-backend-production-5193.up.railway.app/api/brands");
  ...
  const fetchLocations = async () => {
    try {
      const response = await fetch("https://ims-backend-production-5193.up.railway.app/api/locations");
      const data = await response.json();
      setLocations(data);
    } catch (err) {
  ...
  const handleDeleteLocation = async (id) => {
    if (window.confirm("Are you sure you want to delete this location?")) {
      await fetch(`https://ims-backend-production-5193.up.railway.app/api/locations/${id}`, { method: "DELETE" });
      fetchLocations();
    }
  };
